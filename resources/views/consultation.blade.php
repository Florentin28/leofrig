...
<div class="menu">
    ...
    <ul>
        @foreach($locaux as $local) 
            <li><a href="{{ route('consultation.show', ['succursale' => $local->id]) }}" data-local-id="{{ $local->id }}">{{ $local->Nom }}</a></li>
        @endforeach
        <li><a href="{{ route('consultation.index') }}">Toutes les succursales</a></li>
    </ul>
</div>
...
<script>
    ...
    link.addEventListener('click', function(event) {
        ...
        fetch(`/consultation?local_id=${localId}`)
